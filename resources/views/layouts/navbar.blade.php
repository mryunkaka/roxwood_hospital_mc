...
<header class="h-16 bg-surface/80 backdrop-blur-md border-b border-border flex items-center justify-between px-4 lg:px-6 sticky top-0 z-30">
    ...
    <div class="flex items-center gap-3">
        ...
        <nav class="hidden lg:flex items-center gap-2 text-sm">
            <span class="text-text-secondary" data-translate="home">{{ __('messages.home') }}</span>
            <svg class="w-4 h-4 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
            </svg>
            <span class="font-medium text-text-primary" data-translate="page_title">@yield('page-title', 'Dashboard')</span>
        </nav>
    </div>

		    {{-- Right: Language, Theme, Clock, Profile --}}
        ...
        <div class="hidden sm:flex items-center bg-surface-alt rounded-xl p-1 border border-border h-9">
            ...
            <div x-show="open"
                 ...
                 x-cloak>
	                <a href="{{ route('profile') }}" class="flex items-center gap-3 px-4 py-3 hover:bg-surface-hover transition-colors">
	                    <svg class="w-5 h-5 text-text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
	                    </svg>
	                    <span class="text-sm text-text-primary" data-translate="profile_menu">{{ __('messages.profile') }}</span>
	                </a>
                <a href="{{ route('settings') }}" class="flex items-center gap-3 px-4 py-3 hover:bg-surface-hover transition-colors">
                    <svg class="w-5 h-5 text-text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="text-sm text-text-primary" data-translate="settings">{{ __('messages.settings') }}</span>
                </a>
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <button type="submit" class="w-full flex items-center gap-3 px-4 py-3 hover:bg-danger-50 transition-colors text-danger">
                        ...
                        <span class="text-sm" data-translate="logout_menu">{{ __('messages.logout') }}</span>
                    </button>
                </form>
            </div>
        </div>
